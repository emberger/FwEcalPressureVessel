...
void AngularResolution::EresAndShowerprofile(std::string pth){
        ...
        for(Int_t i = 0; i<10; i++) {
                ...
                // fileName=pth+"/gamma"+std::to_string(Estart)+"MeV_0.4:0.4.root";
                // std::unique_ptr<TChain> ch1(new TChain("eventTree"));
                // ch1->Add(fileName.c_str());
                //
                // std::unique_ptr<TTree> EcalTree(ch->GetTree());
                // nofEvents=EcalTree->GetEntries();
                //
                // std::unique_ptr<B4ROOTEvent> Cevent(new B4ROOTEvent());
        ...
        // Estart=1500;
        // fileName=pth+"/gamma"+std::to_string(Estart)+"MeV_.4:.4.root";
        //
        // TFile * f1=new TFile(fileName.c_str());
        // TCanvas * c1 =(TCanvas*)f1->Get("ProjectionCODR");
        // TH2D * hi1 = (TH2D*)c1->GetPrimitive("Projection_minimization");
